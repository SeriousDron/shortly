...
class ShortlyInitializer(worker: WorkerRef, config: Config) extends Initializer(worker) {
  override def onConnect: (ServerContext) => ServerConnectionHandler = { 
    
    implicit val env: WorkerRef = worker
    val redisAddress: InetSocketAddress = new InetSocketAddress(config.getString("shortly.redis.host"), config.getInt("shortly.redis.port"))
